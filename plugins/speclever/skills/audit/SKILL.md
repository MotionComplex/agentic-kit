...
description: >
  ...
  work items, and Figma designs, then ingest the findings into the SpecLever ledger.
  Use when the user says "audit feature X", "/speclever:audit", "validate the spec for X",
...
# /speclever:audit — spec-readiness audit

You are the audit engine of **SpecLever** (a local spec-readiness cockpit; see
...
and ledger data lives per-user in `~/.speclever` (override via `SPECLEVER_DATA`, shared across
repos). All ledger state changes go through the CLI so reconciliation/scoring stay consistent.

## 1. Resolve the feature
- The argument is a `featureId` (kebab-case). Run `SPECLEVER_DATA="${SPECLEVER_DATA:-$HOME/.speclever}" node "${CLAUDE_PLUGIN_ROOT}/app/src/cli.js" feature list --json`.
- If it exists, read `${SPECLEVER_DATA:-$HOME/.speclever}/features/<id>.json` for its `sources` and `specSections`.
- If it does NOT exist: create it with `SPECLEVER_DATA="${SPECLEVER_DATA:-$HOME/.speclever}" node "${CLAUDE_PLUGIN_ROOT}/app/src/cli.js" feature add <id> --title "..."`,
  then add its sources. Discover sources from the user's message (Confluence/ADO/Figma URLs)
  and register each:
  - `SPECLEVER_DATA="${SPECLEVER_DATA:-$HOME/.speclever}" node "${CLAUDE_PLUGIN_ROOT}/app/src/cli.js" source add <id> --type confluence --id <pageId> --title "..." --url "<url>"`
  - `SPECLEVER_DATA="${SPECLEVER_DATA:-$HOME/.speclever}" node "${CLAUDE_PLUGIN_ROOT}/app/src/cli.js" source add <id> --type ado --id <workItemId> --title "..." --url "<url>"` (work item type via `--itemType "User Story"` if available)
  - `SPECLEVER_DATA="${SPECLEVER_DATA:-$HOME/.speclever}" node "${CLAUDE_PLUGIN_ROOT}/app/src/cli.js" source add <id> --type figma --fileKey <key> --nodeId <node> --title "..." --url "<url>"`
...
`specSections` array. If no CLI command exists for this, edit `${SPECLEVER_DATA:-$HOME/.speclever}/features/<id>.json`
...
`SPECLEVER_DATA="${SPECLEVER_DATA:-$HOME/.speclever}" node "${CLAUDE_PLUGIN_ROOT}/app/src/cli.js" coverage set <id> --file coverage.json`.

## 7. Ingest the round
Write the deduped findings array to a temp file and run:
```
SPECLEVER_DATA="${SPECLEVER_DATA:-$HOME/.speclever}" node "${CLAUDE_PLUGIN_ROOT}/app/src/cli.js" ingest <id> --file <tmp.json> --note "<one-line summary of this audit>"
```
...
`SPECLEVER_DATA="${SPECLEVER_DATA:-$HOME/.speclever}" node "${CLAUDE_PLUGIN_ROOT}/app/src/cli.js" readiness <id>` for score + gate + blockers. If there are regressions, call
them out loudly — a fixed thing came back. End by recommending the next step:
- open blockers/majors → **`/speclever:rework`**
- gate `ready` → **`/speclever:brief`**
