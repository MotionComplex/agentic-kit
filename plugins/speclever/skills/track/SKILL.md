...
name: track
description: >
  Give a fast status read on a feature's spec readiness from the SpecLever ledger — score,
  ...
  we on X", "what's still open on X", "/speclever:track", or "spec status".
---

# /speclever:track — fast spec-readiness status
...
```
SPECLEVER_DATA="${SPECLEVER_DATA:-$HOME/.speclever}" node "${CLAUDE_PLUGIN_ROOT}/app/src/cli.js" readiness <id> --json
...
```
Also read `${SPECLEVER_DATA:-$HOME/.speclever}/rounds/<id>.json` for the round history (the last two rounds give you the
deltas).
...
End with the single most useful next action: `/speclever:audit` (if stale / time for a fresh
pass), `/speclever:rework` (open blockers to clear), or `/speclever:brief` (gate is green). Point to
