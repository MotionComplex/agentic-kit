...
name: rework
description: >
  Work through a feature's open SpecLever findings and draft concrete fixes to the
  ...
  issues", "address the open findings", "/speclever:rework", or "let's clear the blockers on X".
---

# /speclever:rework — close the loop on findings

You help the user turn open SpecLever findings into actual edits to the spec and tickets.
...
```
SPECLEVER_DATA="${SPECLEVER_DATA:-$HOME/.speclever}" node "${CLAUDE_PLUGIN_ROOT}/app/src/cli.js" finding list <id> --status open --json
SPECLEVER_DATA="${SPECLEVER_DATA:-$HOME/.speclever}" node "${CLAUDE_PLUGIN_ROOT}/app/src/cli.js" finding list <id> --status reworking --json
```
...
```
SPECLEVER_DATA="${SPECLEVER_DATA:-$HOME/.speclever}" node "${CLAUDE_PLUGIN_ROOT}/app/src/cli.js" finding set <id> <fp> --status reworking
```
Do **not** mark it `resolved` here. Resolution is earned by the next audit: when
`/speclever:audit` re-runs and the issue is genuinely gone, reconciliation auto-resolves it — and
...
```
SPECLEVER_DATA="${SPECLEVER_DATA:-$HOME/.speclever}" node "${CLAUDE_PLUGIN_ROOT}/app/src/cli.js" finding set <id> <fp> --status waived --reason "accepted: ..."
```

## 5. Close out
Summarize what was edited, what was waived, and what still needs a decision. Then recommend
re-running **`/speclever:audit <id>`** to verify the fixes landed, auto-resolve what's done, and
