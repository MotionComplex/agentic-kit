...
**Capture the hidden-by-default surfaces, not just what's on screen at rest — the menu especially.** A resting screenshot shows the *closed* state of everything that opens. The most-missed component is the **navigation menu / overlay**: on most sites the header is a launcher and the real navigation is a full-screen (or drawer) menu that only exists once you open the hamburger — and it is frequently one of the *most designed* surfaces on the site (giant type, its own colour world, per-item hover media, a contact/social block, a close affordance). Treat every opener as a promise of a component: **a hamburger/menu button ⇒ a menu overlay; a "filter"/"cart"/"account" control ⇒ its panel; a "share"/"more" ⇒ its sheet.** Reconstruct these open states as first-class components, not footnotes. When the DOM won't let you trigger the overlay (JS-gated, Lottie/animation buttons that don't respond to a programmatic click), don't skip it — read the menu's markup and CSS directly from the source (its links, numbering, colours, `font-size`, the info block) and rebuild from that. Also capture *decorative-but-signature* hidden elements the opener reveals (e.g. scattered hero image-stickers that fade in). The menu belongs in the component inventory, in `components.html` (shown open), wired into `composition.html`, and in `interactions.json` as an open/close transition.
...
- **Open the menu.** The navigation overlay is a component, not chrome — reconstruct its *open* state (it's often the boldest surface on the site). Never ship a reconstruction whose only navigation is a closed hamburger. The same rule covers any opener→panel pair (cart, filter, search, account).
