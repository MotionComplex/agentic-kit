...
description: >-
  ...
  specimen sheet and ready-to-paste CSS/JS. Use this skill whenever the user
  wants to add, design, define, or systematize animation for an interface —
  "how should this animate", "design a motion system", "add transitions",
  "give me motion tokens", "choreograph the entrance", "make this feel alive",
  "animate these components", "what easing should I use" — or to turn a
  ui-reverse-engineer output (tokens.json + component states) into a motion
  layer. For a STATIC source it infers a motion system from the design's taste;
  for a VIDEO or live URL it can measure real timing/easing via the motion-trace
  MCP. Pairs with ui-reverse-engineer (which extracts the static design system)
  and motion-review (which audits existing motion). Trigger even if the words
  "skill" or "motion system" aren't used, whenever the goal is defining how a
  UI moves rather than how it looks at rest.
---
...
This skill is the motion counterpart to `ui-reverse-engineer`: that skill extracts the *static* design system (the resting state); this one defines the *temporal* layer (how the interface transitions between states). It consumes the same `tokens.json` and the same enumerated component states, and writes motion back into them.
...
- **A static source contains no motion.** A screenshot, mockup, or resting-state design tells you *nothing* about timing or easing. Any motion you produce from it is **inferred from convention and taste**, not measured. Say so. This is a different epistemic basis than `ui-reverse-engineer`'s "measure, don't invent" — here, for static input, you *are* inventing, responsibly.
...
**Mode INFER (static source — the common case).** Input is a `ui-reverse-engineer` output (or any resting-state design + its tokens). You propose a motion system grounded in the design's *taste*: a dense, precise enterprise UI gets fast, restrained, near-linear motion; a playful consumer app gets springier, longer, more expressive motion. The design system itself constrains the answer — base unit, density, accent character, and brand family all imply a motion personality. See `references/motion-inference.md`.

**Mode MEASURE (video / live URL).** Input is a recording or a URL with real animation. Use the **motion-trace MCP** (load via ToolSearch if deferred: `motion_discover`, `motion_capture`, `motion_review`, `motion_diff`, `motion_prompt`) to extract resolved per-frame state — real durations, easing curves, overshoot — the same way `ui-reverse-engineer` Phase 1 Mode A reads computed CSS. Then *systematize* the measured values into tokens exactly as you would raw spacing. See `references/measuring-motion.md`. (The sibling `motion-review` skill audits motion; this skill extracts it into a reusable system.)
...
3. **Per-state transitions on the components** — the transitions *between* the states `ui-reverse-engineer` already enumerated (default ↔ hover ↔ focus ↔ active ↔ disabled, collapsed ↔ expanded, entering ↔ leaving). Motion is the verb connecting the nouns the other skill produced.
...
The mental model mirrors the static skill: **Source → Systematize → Map to states → Choreograph → Build → Verify.**

### Phase 1 — Source the timing (infer or measure)

- **INFER:** read the design's taste from its tokens and from `ui-reverse-engineer`'s `design.html` (family, density, accent, brand). Derive a **motion personality** (e.g. "crisp/functional", "smooth/premium", "bouncy/playful") and the timing implications. Do not pull numbers from thin air — anchor them to the conventions in `references/motion-inference.md` (platform defaults, the 200-300ms "feels instant but visible" band, easing-by-intent).
...
For every component state pair `ui-reverse-engineer` enumerated, define the transition: which properties animate, which role/duration/easing applies, and the reduced-motion fallback. Principles:
...
- **Orchestrate present/dismiss** for overlays (modal, sheet, popover) including the scrim, and respect the size-class substitutions from `ui-reverse-engineer` (a bottom sheet and a centered dialog present differently).
